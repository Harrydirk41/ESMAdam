...
def gen_quat_torch(num_quaternions, quat_in = None, device = "cuda"):
    ## 
...
def generate_images_simple(
    ...
    if rotation:
        if input_quat is not None:
            quats = gen_quat_torch(N_images, input_quat.to(device), device)
        else:
            quats = gen_quat_torch(N_images, None,device)
        rot_mats = quaternion_to_matrix(quats).type(torch.float64)
        rot_mats = rot_mats.to(device)
        coord_rot = coord.matmul(rot_mats)
    else:
